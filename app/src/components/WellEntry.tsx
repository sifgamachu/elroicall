      ...
      <div className="relative mx-auto flex min-h-svh max-w-7xl flex-col px-6 pb-16 pt-28 lg:px-10">
        <div className="grid flex-1 items-center gap-10 lg:grid-cols-[0.82fr_1.18fr] lg:gap-16">
          <div className="relative z-10 order-2 max-w-xl lg:order-1">
            <div className="mb-8 inline-flex items-center gap-3 border-b border-[#d4ae62]/40 pb-3 text-sm font-medium uppercase tracking-[0.34em] text-[#d9b86f]">
            ...
            <p className="mt-7 max-w-lg text-base font-light leading-[1.85] text-[#b9ad98] sm:text-lg">
              You do not need the right words. Share what is on your mind with an AI guide, then explore a story from Scripture together.
            </p>
            <blockquote className="font-serif-display mt-8 max-w-md border-l border-[#cda85d]/70 pl-5 text-xl font-light italic leading-relaxed text-[#e9dfcb]">
              ...
              <span className="mt-2 block font-sans text-xs not-italic uppercase tracking-[0.12em] text-[#a99368]">Genesis 16:13</span>
            </blockquote>
          </div>

          <div className="relative order-1 mx-auto w-full max-w-2xl py-2 lg:order-2 lg:py-14">
            ...
            <div className="relative z-10 mx-auto max-w-xl border border-[#d5b46d]/35 bg-[#11100d]/92 p-5 shadow-[0_40px_120px_rgba(0,0,0,.5)] backdrop-blur-xl sm:p-7">
              <div className="flex flex-wrap items-center justify-between gap-3">
                <p className="text-sm font-medium uppercase tracking-[0.12em] text-[#d8b56b]">At the edge of the Well</p>
                <span className="inline-flex items-center gap-1.5 text-sm text-[#8e836f]"><LockKeyhole className="h-3 w-3" /> private draft</span>
              </div>
              ...
              <div className="mt-5 grid grid-cols-2 gap-2 sm:grid-cols-3">
                {NEEDS.map((need) => (
                  <button
                    ...
                    className="min-h-11 border border-white/[0.08] bg-white/[0.025] px-3 py-2.5 text-left text-sm font-light text-[#c9bdab] transition-colors hover:border-[#d5b46d]/55 hover:bg-[#d5b46d]/[0.07] hover:text-[#f5eddc]"
                  >
                    {need.label}
                  </button>
                ))}
              ...
              <button
                ...
                className="group mt-5 flex w-full items-center justify-between bg-[#d6b267] px-5 py-4 text-left text-sm font-semibold uppercase tracking-[0.1em] text-[#17130d] transition-all hover:bg-[#e6c77e] disabled:cursor-not-allowed disabled:opacity-35"
              >
                <span>Continue with what I wrote</span>
                <ArrowRight className="h-4 w-4 transition-transform group-hover:translate-x-1" />
              </button>
              <p className="mt-4 text-sm font-light leading-relaxed text-[#8f8371]">
                Your draft stays in this tab. Review it and agree to AI use and privacy before submitting. Refreshing or closing the page clears an unsubmitted draft.
              </p>
            </div>

            <a href={PHONE_TEL} className="relative z-10 mx-auto mt-8 flex w-fit items-center gap-3 text-sm font-medium uppercase tracking-[0.1em] text-[#9e917c] transition-colors hover:text-[#e1bd6e]">
              <Phone className="h-3.5 w-3.5" /> Prefer voice · {PHONE_DISPLAY}
            </a>
          </div>
        </div>

        <a href="#descent" className="mx-auto mt-6 flex flex-col items-center gap-2 text-xs font-medium uppercase tracking-[0.12em] text-[#766d60] transition-colors hover:text-[#d8b56b]">
          <span>Go deeper</span>
