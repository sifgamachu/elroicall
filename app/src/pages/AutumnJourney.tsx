...
function DayReader({reading,completed,toggle,notice}:{reading:AutumnReading;completed:number[];toggle:(day:number)=>void;notice:string}) {
  ...
  return <div className="autumn-reader"><Link className="autumn-text-link" to={`/journey/#week-${Math.ceil(reading.day/7)}`}><ArrowLeft size={17}/>All readings</Link><div className="autumn-reader-progress"><span>DAY {String(reading.day).padStart(2,'0')} OF 84</span><progress value={completed.length} max={84} aria-label={`${completed.length} of 84 readings complete`}/></div><img className="autumn-reader-image" src="/images/autumn-valley-1280.webp" width="1280" height="720" alt="Autumn waterfalls and a sunlit mountain valley"/><header><p className="autumn-eyebrow">WEEK {Math.ceil(reading.day/7)} · {AUTUMN_WEEKS[Math.floor((reading.day-1)/7)]}</p><h1>{reading.day===1?'When everything began':reading.day===84?'All things made new':`Day ${reading.day}: ${reading.concept.split(':')[0]}`}</h1><p>{reading.passage.replaceAll('-', '–')}</p></header>
...
export default function AutumnJourney() {
  ...
  return <AutumnShell><section className="autumn-width autumn-journey-intro"><p className="autumn-eyebrow">OCTOBER — DECEMBER 2026</p><h1>One story.<br/>A place for every day.</h1><p>Read all 66 books in 84 reading days, with 8 flexible days before the year ends. Start whenever you’re ready. Every day is open.</p><Link className="autumn-button" to={`/journey/${next?.day||1}/`}>{completed.length===84?'Revisit Day 1':completed.length?`Continue with Day ${next?.day}`:'Begin with Day 1'}<ArrowRight size={18}/></Link><div className="autumn-journey-progress"><span>{completed.length} of 84 readings complete</span><progress value={completed.length} max={84} aria-label="Reading progress"/><p>Progress stays in this browser. No account needed to read.</p></div></section><div className="autumn-width autumn-weeks">{AUTUMN_WEEKS.map((title,index)=><section id={`week-${index+1}`} className="autumn-week" key={title}><div><p className="autumn-eyebrow">WEEK {String(index+1).padStart(2,'0')}</p><h2>{title}</h2></div><ol>{AUTUMN_READINGS.slice(index*7,index*7+7).map(item=><li key={item.day}><Link to={`/journey/${item.day}/`}><span className="autumn-day-number">{completed.includes(item.day)?<Check size={18} aria-label="Completed"/>:String(item.day).padStart(2,'0')}</span><span><small>DAY {item.day}</small>{item.passage.replaceAll('-','–')}</span><ArrowRight size={17}/></Link></li>)}</ol></section>)}</div><section className="autumn-width autumn-journey-close"><BookOpen/><p>This plan covers the 1,189 chapters of the 66-book Protestant Bible. Daily assignments follow your 84-day reading challenge. Overviews help you prepare; they do not replace reading Scripture.</p><Link className="autumn-text-link" to="/explore/">Looking for a shorter reflection?<ArrowRight size={17}/></Link></section></AutumnShell>;
}
