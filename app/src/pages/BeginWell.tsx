...
export default function BeginWell() {
  ...
  return (
    <ProductShell>
      <div className="elroi-intake-layout mx-auto grid max-w-6xl">
        <aside className="elroi-intake-aside lg:sticky lg:top-28 lg:self-start">
          <p className="text-sm font-semibold text-gold">Your first conversation</p>
            ...
            {LEVELS.map((level, index) => {
              ...
              return (
                <div key={level.id} aria-current={index === activeIndex ? "step" : undefined} className="relative flex gap-3">
                  <span className={`relative z-10 mt-1 h-3.5 w-3.5 rounded-full border ${active ? "border-gold-soft bg-[hsl(var(--gold))]" : "border-slate-200 bg-slate-200"}`} />
                  <div>
                    <p className={`text-sm font-semibold ${active ? "text-gold-bright" : "text-parchment-dim"}`}>{level.label}</p>
                    <p className="mt-1 hidden text-sm font-normal text-parchment-dim lg:block">{level.note}</p>
                  </div>
                </div>
              );
            })}
        ...
        <div ref={heading} tabIndex={-1} className="min-w-0 outline-none">
          {stage === "surface" && (
            <section>
              <p className="eyebrow">Your words first</p>
              <h1 className="font-serif-display mt-5 text-4xl font-normal leading-[1.02] text-parchment sm:text-6xl">{copy.title}</h1>
              <p className="mt-5 max-w-2xl text-base font-normal leading-[1.85] text-parchment-dim">{copy.prompt}</p>
              ...
              <textarea
                ...
                className="font-serif-display mt-3 w-full resize-y border border-slate-200 bg-white px-5 py-5 text-xl font-normal leading-[1.65] text-parchment outline-none placeholder:text-parchment-dim/50 focus:border-gold-soft sm:text-2xl"
              />
              ...
              <div className="mt-5 border border-slate-200 bg-white p-5">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <p className="text-sm font-semibold text-gold">Before this leaves your browser</p>
                    <p className="mt-2 text-sm font-normal leading-[1.75] text-parchment-dim">
                ...
                <label className="mt-4 flex cursor-pointer items-start gap-3 text-sm font-normal leading-relaxed text-parchment-dim">
              ...
              {error && <div role="alert" className="mt-4 text-sm elroi-status-error"><p>{error}</p><a href={PHONE_TEL} className="mt-3 inline-flex min-h-11 items-center gap-2 text-gold-bright underline"><Phone className="h-4 w-4" />Call instead · {PHONE_DISPLAY}</a></div>}
              <button type="button" disabled={burden.trim().length < 3 || !consented} onClick={reflect} className="group mt-6 flex w-full items-center justify-between bg-[hsl(var(--gold))] px-6 py-4 text-sm font-semibold text-white hover:bg-[hsl(var(--gold-bright))] disabled:cursor-not-allowed disabled:opacity-35">
                <span>Reflect with El Roi</span><ArrowRight className="h-4 w-4 transition-transform group-hover:translate-x-1" />
              </button>
              <p className="mt-5 text-center text-sm font-normal leading-relaxed text-parchment-dim">If you may hurt yourself or someone else, or are in immediate danger, use human emergency support instead. In the U.S., call or text 988, or call 911.</p>
            </section>
          )}

          {stage === "listening" && (
            <section role="status" aria-live="polite" className="py-20 text-center">
              <div className="mx-auto h-24 w-24 rounded-full border border-gold-soft/25 p-4"><div className="breathe h-full w-full rounded-full border border-gold-soft/45 bg-indigo-50" /></div>
              <p className="font-serif-display mt-8 text-4xl font-normal italic text-parchment">Stay here for a moment.</p>
              <p className="mx-auto mt-4 max-w-md text-base font-normal leading-[1.8] text-parchment-dim">Your AI reflection is being prepared. This may take a few seconds.</p>
          ...
          {stage === "seen" && (
            <section>
              ...
              <h1 className="font-serif-display mt-5 text-4xl font-normal text-parchment sm:text-5xl">Here is what El Roi heard.</h1>
              <blockquote className="font-serif-display mt-7 border-l border-gold-soft pl-5 text-2xl font-normal italic leading-[1.6] text-parchment sm:text-3xl">“{reflection}”</blockquote>
              ...
              <div className="mt-10 border border-gold-soft/35 bg-indigo-50 p-6 sm:p-8">
                <p className="text-sm font-semibold text-gold">A story worth opening</p>
                <h2 className="font-serif-display mt-3 text-5xl font-normal italic text-gold-bright">{storyName}</h2>
                <p className="mt-4 max-w-2xl text-sm font-normal leading-[1.8] text-parchment-dim">
              ...
              <form onSubmit={(event) => { event.preventDefault(); void claim(); }} aria-busy={claiming} className="mt-8 border-t border-slate-200 pt-7">
                ...
                <input id="claim-phone" type="tel" required disabled={claiming} aria-describedby="phone-help phone-error" aria-invalid={Boolean(phone.trim() && !normalizedPhone)} value={phone} onChange={(event) => { setPhone(event.target.value); setError(""); }} inputMode="tel" autoComplete="tel" placeholder="(202) 555-0123" className="mt-3 w-full border border-slate-200 bg-white px-5 py-4 text-lg text-parchment outline-none placeholder:text-parchment-dim/50 focus:border-gold-soft" />
                <p id="phone-error" aria-live="polite" className="mt-3 text-sm elroi-status-error">{phone.trim() && !normalizedPhone ? "Enter a complete phone number, including country code for international numbers." : ""}</p>
                {error && <div role="alert" className="mt-3 text-sm elroi-status-error"><p>{error}</p><a href={PHONE_TEL} className="mt-3 inline-flex min-h-11 items-center gap-2 text-gold-bright underline">Call instead · {PHONE_DISPLAY}</a></div>}
                <button type="submit" disabled={!normalizedPhone || claiming} className="group mt-5 flex w-full items-center justify-between bg-[hsl(var(--gold))] px-6 py-4 text-sm font-semibold text-white hover:bg-[hsl(var(--gold-bright))] disabled:opacity-35">
          ...
          {stage === "ready" && (
            <section className="py-8 text-center">
              ...
              <h1 className="font-serif-display mt-5 text-5xl font-normal text-parchment sm:text-6xl">Ready when you are.</h1>
              <p className="mx-auto mt-5 max-w-xl text-base font-normal leading-[1.85] text-parchment-dim">Call from <strong className="font-medium text-parchment">{preparedPhone}</strong> so the phone service can match your starting words. You can change direction, decline prayer, or end the conversation at any time.</p>
              <a href={PHONE_TEL} className="mt-8 inline-flex min-h-14 items-center justify-center gap-3 bg-[hsl(var(--gold))] px-10 py-4 text-base font-semibold text-white hover:bg-[hsl(var(--gold-bright))]"><Phone className="h-5 w-5" />Call now</a>
              <a href={PHONE_TEL} className="font-serif-display mt-5 block text-3xl font-normal text-gold-bright hover:text-parchment sm:text-4xl">{PHONE_DISPLAY}</a>
