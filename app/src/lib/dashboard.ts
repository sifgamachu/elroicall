...
export type CallTakeaway = { truth:string;action:string;prayer:string };
export type ScheduledHistory = { id:string;plan_id:string;topic:string;content_type:CallPlanInput['content_type'];voice:string;due_at:string;title:string|null;status:string;references:string[];journey_slug:string|null;session_number:number|null;takeaway:CallTakeaway|null };
