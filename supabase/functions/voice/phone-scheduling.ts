...
When someone wants a future call, first use check_bible_call_scheduling to check availability before collecting details. Offer either a seven-call daily journey or a custom call. Available journeys are Peace in Seven Days, Held Through Grief, Rest With Scripture, Purpose for This Season, Courage for What Is Ahead, and Bible Foundations. For a journey, use its exact journey_slug, set journey_total to 7, content_type to bible_study, weekdays to all seven days, and topic to the caller's chosen need in their own words. Otherwise help them choose: Bible study, sermon, Bible lecture, biblical story, or Bible facts; their own topic or passage; Marin, Cedar, Coral, or Onyx AI voice; a 5, 10, or 15 minute length; exact local time including AM/PM; their city/time zone; first calendar date; and once or selected weekly days. Daily means all seven days. Ask short questions for missing choices; do not silently choose a topic, voice, date, or time zone. Voice choices are narrator names, not biblical figures or imitations. Use the current date below to resolve relative dates, and confirm the exact date aloud. Phone bookings require at least forty minutes of notice.
...
const planProperties={
  ...
  journey_slug:{type:'string',enum:['peace','grief','sleep','purpose','courage','foundations'],description:'Include only for a seven-call guided journey.'},
  journey_total:{type:'integer',const:7,description:'Include only with journey_slug.'},
};
const requiredPlan=['content_type','topic','voice','local_time','timezone','start_date','recurrence','weekdays','duration_minutes'];
export const PHONE_SCHEDULING_TOOLS=[
  ...
  {name:'prepare_bible_call',description:'Prepare and read back the caller’s chosen learning schedule or seven-call journey. Does not save it. Collect every choice first. Do not announce a booking before or after this tool.',input_schema:{type:'object',additionalProperties:false,properties:planProperties,required:requiredPlan}},
...
export async function phoneSchedulingTurn(base:string,key:string,callSid:string,state:PhoneBookingState,name:string,input:unknown,client:typeof fetch=fetch):Promise<string>{
  ...
  try{
    if(name==='check_bible_call_scheduling'){
      ...
      return r.ok&&capabilities.ready===true?'Phone scheduling is available. Would you like a seven-call journey for peace, grief, rest, purpose, courage, or Bible foundations—or a custom Bible study, sermon, lecture, story, or Bible facts call?':'Scheduled learning calls are not accepting bookings yet. You can still call El Roi anytime for a conversation.';
    }
